...
namespace EntityFrameworkCore.UnitOfWork
{
    ...
    public class UnitOfWork : IUnitOfWork
    {
        ...
        public async Task<IList<T>> FromSqlAsync<T>(string sql, IEnumerable<object> parameters = null, CancellationToken cancellationToken = default) where T : class
        {
            ...
            var entities = await dbSet.FromSql(sql, parameters ?? Enumerable.Empty<object>()).ToListAsync(cancellationToken).Then<List<T>, IList<T>>(result => result, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
